...
class Polls:
    ...
    async def poll(self, ctx, *, question):
        ...
        tellMeE = discord.Embed(title='Poll creator', description=f'Tell me a possible choice of the poll or \nmake `-publish` to publish the poll or\n`-cancel` to cancel the poll', color=0x00ff00)
        ...
        for i in range(0, 20):
            ...
            if possibleChoice.clean_content.startswith('-publish'):
                publish = True
                break

            elif possibleChoice.clean_content.startswith('-cancel'):
                publish = False
                break

            answers.append(possibleChoice.clean_content)
            tellMeE = discord.Embed(title='Poll creator', description=f'Your choice n°{len(answers)} ({answers[i]}) has been saved !\nTell me a possible choice of the poll or \nmake `-publish` to publish the poll or\n`-cancel` to cancel the poll', color=0x00ff00)
            tellMeE.set_author(name=self.bot.user.name, icon_url=self.bot.user.avatar_url)
            tellMeE.set_footer(text=f'-poll command requested by {ctx.message.author}', icon_url=ctx.message.author.avatar_url)
        ...
        if publish == True:
            pollEmbed = discord.Embed(title=f'{ctx.message.author} asks', description=f'*{question}*', color=0x00ff00)
            ...
            cancelE = discord.Embed(title='Poll creator', description=f'{ctx.message.author} your poll has been cancelled', color=0x00ff00)
            cancelE.set_author(name=self.bot.user.name, icon_url=self.bot.user.avatar_url)
            cancelE.set_footer(text=f'poll cancel requested by {ctx.message.author}', icon_url=ctx.message.author.avatar_url)
